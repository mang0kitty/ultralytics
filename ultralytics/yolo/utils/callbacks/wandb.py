...
def on_pretrain_routine_start(trainer: BaseTrainer):
    ...
    Parameters
    ----------
    trainer : `:class:ultralytics.yolo.engine.trainer.BaseTrainer`
        A task trainer that's inherited from `:class:ultralytics.yolo.engine.trainer.BaseTrainer`
...
def on_train_end(trainer: BaseTrainer):
    # wandb.save(
    #     glob_str=str(trainer.save_dir / "weights" / "*.pt"), base_path=trainer.save_dir
    # )
